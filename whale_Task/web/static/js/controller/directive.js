...
whaleModule.directive('orderList',["$rootScope","$http","$timeout",function($rootScope,$http,$timeout){
    var linkFunction=function(scope,element,attr){
        function httpquery(index,flag){
            ...
            $http.get("/whaleApiMgr/taskReward/totalCount",{
                params: {
                    start:whale.store("starttime"),
                    end:whale.store("endtime"),
                    PageIndex:index,
                    PageSize:5
                }
...
whaleModule.directive('orderList1',["$rootScope","$http","$timeout",function($rootScope,$http,$timeout){
    var linkFunction=function(scope,element,attr){
        function httpquery(index,flag){
            scope.picloading=true;
            $timeout(function(){
            ...
            $http.get("/whaleApiMgr/taskReward/selectAllReward",{
                params: {
                    apiName:whale.store("taskName"),
                    PageIndex:index,
                    PageSize:5
                }
            }).success(function (data) {
    ...
    return {
        restrict: "E",
        replace:true,
        templateUrl: "static/template/orderList1.html",
        link: linkFunction
    }
}])
