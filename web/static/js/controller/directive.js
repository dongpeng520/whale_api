    ...
    var linkFunction=function(scope,element,attr){
        function httpquery(pin,index,flag){
            $http.get("/task/taskFileStorageconroller/queryFileStorage"+"?accessToken="+whale.store("accessToken"),{
                ...
                if (data.code == 10200) {
                    scope.order=data.data;
                    $rootScope.$broadcast('history.page', data.total,5,flag);  //发送给pagemiddle  页码长度
                }
            }).error(function(data) {
                if (data.code == 41400) {
                    $rootScope.errormsg = '此用户在另一设备登录，请重新登录';
...
whaleModule.directive('lazySrc', ['$document', '$timeout','$compile', '$rootScope',
    function($document, $timeout, $compile, $rootScope) {
        'use strict';
        return {
            restrict: 'AEC',
            template: '<img image-lazy-src>',
            scope: {
                imageSrc: "@"
            },
            replace:true,
            link: function($scope, $element, $attributes) {
                ...
                $scope.$broadcast('lazySrcEvent', $scope.srcObject);
            }
        };
    }
])
whaleModule.directive('imageLazySrc', ['$document', '$timeout',  '$compile',
    function($document, $timeout, $compile) {
        'use strict';
        return {
            restrict: 'A',
            link: function($scope, $element, $attributes) {
                $scope.$on('lazySrcEvent', function(d, data) {
                ...
                function imgOnloadPhotoBar(ele){
                    ...
                    var www=ele.parentNode.clientWidth;
                    var hhh=ele.parentNode.clientHeight;
